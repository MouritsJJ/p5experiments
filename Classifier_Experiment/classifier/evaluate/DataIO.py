...
class DataIO:
    ...
    def save_single_image(self, img, image_name, title = ''):
        ...
        plt.savefig(f'{self.path}/{image_name}.png', bbox_inches=0)
        plt.close()
    
    def save_prediction_images(self, correct_preds, wrong_preds, images):
        for label in classes:
            os.makedirs(f'{self.path}/classified_images/correct/{label}', exist_ok=True)
            os.makedirs(f'{self.path}/classified_images/wrong/{label}', exist_ok=True)

        for image in correct_preds:
            self.save_single_image(denormalize(images[image[0]].cpu()), 
                f'classified_images/correct/{classes[image[1]]}/{self.img_num}', f'Prediction {classes[image[2]]}')
            self.img_num += 1

        for image in wrong_preds:
            self.save_single_image(denormalize(images[image[0]].cpu()), 
                f'classified_images/wrong/{classes[image[1]]}/{self.img_num}', f'Prediction {classes[image[2]]}')
            self.img_num += 1
